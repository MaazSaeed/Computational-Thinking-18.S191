...
# ╔═╡ 79a8148f-aabe-458b-9a4b-664dc801d3b9
random_vect
...
# ╔═╡ 5ef5b271-5f8e-4b34-95eb-a8a1542f5427
demean(random_vect)
...
function create_bar()
	zeros = [0 for i=1:100] # Generating a list with 100 zeros
	zeros[40:60].=1; # Broadcasting
	return zeros
end
...
function matrix_to_vecvec(matrix)
	# i-th row and the entire column
...
function mean_colors(image)
	total_pixels = prod(size(image)) # width * height
	(r, g, b) = (0.0, 0.0, 0.0)
...
begin
	...
	function quantize(color::AbstractRGB)
		...
		return RGB(quantize(float(color.r)), quantize(float(color.g)), quantize(float(color.b)))
	end
	...
	function quantize(image::AbstractMatrix)
		...
		for i in 1:size(image, 1)
			for j in 1:size(image, 2)
				img_cpy[i, j] = quantize(image[i, j])
			end
		end
		return missing
	end
...
# ╔═╡ 0d320864-067c-4cd3-a957-80feac1e683e
rand(1)
...
begin
	...
	function noisify(x::Number, s)
		return myclamp(x + -s + rand(Float64) * 2 * s)
	end
...
# ╔═╡ 1e1e0b06-3a25-4fc8-9552-57506a727c09
...
function blur_1D(vec, l)
	blur_vec = Vector{Float64}(undef, n)
	for i in 1:length(vec)
		box_start = max(1, i - div(l, 2))
		box_end   = min(n, i + div(l, 2))
		sz = box_end - box_start + 1
		t = sum(vec[box_start : box_end])
		blur_vec[i] = t / sz
	end
	return blur_vec
end
...
# ╔═╡ 989e5b6d-102f-402c-9d99-b03f593f46ec
...
function convolve_vector(v, k)
	conv_vec = Vector{Float64}(undef, length(v))
	l = (length(k) - 1) ÷ 2
	for i in 1:length(v)
		conv_vec[i] = sum([j for j in extend(v, (i-l):(i+l))].* k)
	end
...
function gaussian_kernel(n, σ=1)
	x = -n ÷ 2 : n ÷ 2
	gaussian = (1 / √(2 * π * σ^2)) .* exp.(-x.^2 / (2 * σ^2))
...
function extend_mat(M::AbstractMatrix, i, j)
	...
	return M[max(1, min(num_rows, i)), max(1, min(num_cols, j))]
end
...
begin	
	function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
		(nrows_K, ncols_K) = size(K)
		(nrows_M, ncols_M) = size(M)

		win(idx, bound) = (idx - (bound - 1) ÷ 2) : (idx + (bound - 1) ÷ 2)
		conv(i, j) = sum([extend_mat(M, k, l) for k in win(i, nrows_K), l in win(j, ncols_K)] .* K)
		
		# for each pixel call the conv function
		# conv function then takes a dot product of the
		# kernel and the corresponding pixels
		# sums up the result and returns the sum.
		return [conv(i, j) for i=1:nrows_M, j=1:nrows_M]
	end
end

# ╔═╡ 8d563722-145b-49cf-9204-abc9f4508820
for i in 0:5
	print(i)
	println()
end
...
function with_gaussian_blur(image)
	gaussian_2D(i, j, σ) = (1 / (2 * π * σ^2)) * exp(-(i^2 + j^2) / (2 * σ^2))
	kernel(n, σ) = [gaussian_2D(i - (n + 1) ÷ 2, j - (n + 1) ÷2, σ) for i=1:n, j=1:n]
	return convolve_image(image, kernel(21, 1))
end
